...
export default function Analytics() {
  ...
  const adjusted = useMemo(() => {
    const factor = (filters.region === 'EMEA' ? 1.06 : filters.region === 'APAC' ? 1.12 : 1.0)
      ...
      <Card title="Filters">
        <div className="form-row">
          ...
          <div className="input">
            ...
            <input type="range" min="3" max="12" value={filters.horizon}
      ...
      <section style={{marginTop:16}} className="grid-2">
        ...
        <Card title="Projection Table">
          <table className="table">
            <thead>
              <tr><th>Month</th><th>Projected</th></tr>
            </thead>
            <tbody>
              {adjusted.map((v,i)=>(
                <tr key={i}><td>M{i+1}</td><td>${v}k</td></tr>
              ))}
            </tbody>
          </table>
        </Card>
